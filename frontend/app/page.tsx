...
const StockDashboard = () => {
  ...
  useEffect(() => {
    fetchStocks();
    const interval = setInterval(fetchStocks, 10000); // Refresh every 10s for "real-time" feel
    return () => clearInterval(interval);
  }, []);

  if (loading) return <div className="flex h-screen items-center justify-center bg-slate-900 text-white font-mono">LOADING_MARKETS...</div>;

  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 p-8 font-sans">
      <header className="mb-12 flex justify-between items-end">
        <div>
          <h1 className="text-4xl font-bold text-white tracking-tight">Quant<span className="text-blue-500">View</span></h1>
